...
//lab3-3:添加复制函数
//pagetable：起始页表（用户进程页表）
//kernelpt：目标页表（进程内核页表）
//oldsz：旧用户空间大小 newsz：新用户空间大小（字节）
void
u2kvmcopy(pagetable_t pagetable, pagetable_t kernelpt, uint64 oldsz, uint64 newsz){
  pte_t *pte_from, *pte_to;
  oldsz = PGROUNDUP(oldsz);//向上舍入到下一个页大小，确保从整个页复制数据
  for (uint64 i = oldsz; i < newsz; i += PGSIZE){ //每次增加一个页面大小
    if((pte_from = walk(pagetable, i, 0)) == 0) 
      panic("u2kvmcopy: src pte does not exist");
    if((pte_to = walk(kernelpt, i, 1)) == 0)
      panic("u2kvmcopy: pte walk failed");
    uint64 pa = PTE2PA(*pte_from); //页表pte_from转为对应物理地址pa
    uint flags = (PTE_FLAGS(*pte_from)) & (~PTE_U);//获取标志位并将标志位取反
    *pte_to = PA2PTE(pa) | flags;//pte_to指向的页表条目设置为pa物理地址
  }
}
